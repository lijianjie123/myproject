				...
				<div class="item-box">
					<div class="gallery-wrapper">
						<div class="gallery">
							<div class="thumbnail">
								<ul>
									<li :class="{'on':index === itemIndex}" 
                  v-for = "(img,index) in itemInfo.ali_images" :key = "index" 
						...
						<div class="sku-dynamic-params-panel">
							<div class="sku-dynamic-params clear">
								...
								<ul class="params-colors">
								  <li :class = "{'cur':color.id == itemId }" 
                  v-for = "(color,index) in itemInfo.sku_list" :key = "index" >
							...
							<div class="sku-dynamic-params clear">
								...
								<div class="params-detail clear">
									<div class="item-num js-select-quantity">
										<span class="down" :class = "{'down-disabled': this.count === 1 }" 
                    @click = "subCount">-</span>
										<span class="num">{{this.count}}</span>
										<span class="up" :class = "{'up-disabled': this.count === this.itemInfo.limit_num }"
                     @click = "addCount">+</span>
									</div>
								</div>
							</div>
...
<script>
import itemsData from "@/lib/newItemsData"; // 导入这个页面使用到的数据文件
...
export default {
  data() {
    return {
      // itemsData  是导入的数据文件
      itemsData, //: itemsData, // 冒号前的是自己起的名字， 冒号后是导入的数据  也可以缩写为itemsData
      // 路由信息对象下拿到参数或者字符串参数  this.$route.params.id   this.$route.query.itemId
      itemId: this.$route.query.itemId, // 初始化话赋值的   所以想要监控其值的改变  要使用watch() 方法
      itemIndex: 0,
      count: 1
    };
  },
  ...
  watch: {
    // 监控的是传递过来的id(路由信息对象下的参数  $route.query.itemId )  若果 这个值有改变  就重新给itemId 赋值
    "$route.query.itemId"() {
  ...
  computed: {
    itemInfo() {
      ...
      // itemInfo是个数组 里面只有一个 而且要取第0个出来
      let itemInfo = this.itemsData.filter(function(item) {
        return Number(item.sku_id) === Number(self.itemId);
      })[0];
      return itemInfo; //必须要return出去   不然  这个方法是拿不到数据的
    }
    // itemInfo() {
  ...
  methods: {
    ...
    // 加入购物车
    addCarPanelHandle() {
      let itemData = { info: this.itemInfo, count: this.count }; //这里给data 做下修改 把data改为对象
      this.$store.commit("addCarPanelData", itemData);
      // this.$store.commit("addCarPanelData", this, itemInfo);
    },
    addCount() {
      this.count++;
