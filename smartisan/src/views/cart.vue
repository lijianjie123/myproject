            ...
            <div class="cart-box">
                ...
                <div class="cart-inner">
                    <div class="empty-label " v-if="totleCount<=0">
                        <h3>您的购物车中还没有商品</h3>
                        <router-link class="link" :to = "{name:'Shop'}">现在选购</router-link>
                    </div>
                    <div v-else>
                        ...
                        <div class="cart-table">
                            <div class="cart-group">
                                <!--购物列表-->
                                <div class="cart-top-items" v-for = "(carPanel,index) in carPanelData" :key="index" >
                                    <div class="cart-items">
                                        <div class="items-choose">
                                            <span class="blue-checkbox-new " :class = "{'checkbox-on':carPanel.checked }" 
                                            @click = "checkgoodsHandle(carPanel.sku_id)"><a></a></span>
                                        </div>
                                        <div class="items-thumb">
                                            <img :src="carPanel.ali_image">
                                            <a href="javascript:;" target="_blank"></a>
                                        </div>
                                        <div class="name hide-row" >
                                            <div class="name-table">
                                                <a href="javascript:;" target="_blank">{{carPanel.title}}</a>
                                                <ul class="attribute">
                                                    <li>{{carPanel.spec_json.show_name}}</li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="operation">
                                            <a class="items-delete-btn" @click = "delCarPanelData(carPanel.sku_id)"></a>
                                        </div>
                                        <div class="subtotal">¥ {{carPanel.price*carPanel.count}}</div>
                                        <div class="item-cols-num">
                                            <div class="select js-select-quantity">
                                                <span class="down " :class = "{'down-disabled': carPanel.count <= 1 }" @click="subCarpanelData(carPanel.sku_id)">-</span>
                                                <span class="num">{{carPanel.count}}</span>
                                                <span class="up" :class = "{'up-disabled': carPanel.count >= carPanel.limit_num }" @click="plusCarpanelData(carPanel.sku_id)">+</span>
                                                
                                            </div>
                                        </div>
                                        <div class="price">¥ {{carPanel.price}}</div>
                                    </div>
                                </div>
                    ...
                    <div class="cart-bar-operation">
                        <div>
                            <div class="choose-all js-choose-all">
                                <!--  -->
                                <span class="blue-checkbox-new " :class = "{'checkbox-on':allchecked}" 
                                @click = "allcheckgoodsHandle(allchecked)"><a></a></span>
                                全选
                            </div>
                            <div class="delete-choose-goods" @click = "delGoodsChecked">删除选中的商品</div>
                        </div>
                    </div>
                    <div class="shipping">
                        ...
                        <span class="jianguo-blue-main-btn big-main-btn js-checkout " :class = "{'disabled-btn':totalChecked <=0}"><router-link :to = "{name:'CheckOut'}">现在结算</router-link></span>
                    </div>
                </div>
            </div>
        </div>
...
export default {
  computed: {
    ...
    // totalPrice
    allchecked() {
      //根据这个来判断  是否为全部选中    这个值的结果  就是  true 或false
  ...
  methods: {
    ...
    checkgoodsHandle(id) {
      this.$store.commit("checkedGoods", id);
    },
    allcheckgoodsHandle(allchecked) {
      this.$store.commit("allcheckedCarpanel", allchecked);
    },
    delGoodsChecked() {
      this.$store.commit("delGoodsChecked");
    }
  }
};
</script>
