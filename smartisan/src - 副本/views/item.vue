        ...
        <!-- 商品详情 -->
			<div class="store-content item">
				<div class="item-box">
                  ...
                  :key = "index" @click = "tableImg(index)">
                  <img 
                  :src="img+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'">
                  </li>
								</ul>
							</div>
							<div class="thumb">
                  ...
                  :key = "index" >
                    ...
                    @click = "subCount">-</span>
										...
										<span class="up" 
                    :class = "{'up-disabled': this.count === this.itemInfo.limit_num }" 
                    @click = "addCount">+</span>
									</div>
								</div>
							</div>
...
export default {
  data() {
    ...
    return {
      itemsData, //接收一下 这样页面中就能应用数据了
      itemId: this.$route.query.itemId, //$route是路由信息对象   {{ $route.query}}路由信息对象参数都在$route.query下
      itemIndex: 0,
      count: 1
    };
  },
