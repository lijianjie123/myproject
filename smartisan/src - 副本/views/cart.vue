<template>
	<div id="main" class="hander-car">
		<div class="store-content">
					...
					<div v-else>
						...
						<div class="cart-table">
							<div class="cart-group">
								<!--购物列表-->
								<div class="cart-top-items" v-for = "(CarGoogs,index) in carPanelData" :key = "index">
									<div class="cart-items">
										<div class="items-choose">
											<span class="blue-checkbox-new" :class = "{'checkbox-on': CarGoogs.checked}"  @click="checkgoodsHandle(CarGoogs.sku_id)"><a></a></span>
										</div>
										<div class="items-thumb">
											<img :src="CarGoogs.ali_image +'?x-oss-process=image/resize,w_440/quality,Q_90/format,webp'">
											<a href="javascript:;" target="_blank"></a>
										</div>
										<div class="name hide-row" >
											<div class="name-table">
												<a href="javascript:;" target="_blank">{{CarGoogs.title}}</a>
												<ul class="attribute">
													<li>{{CarGoogs.spec_json.show_name}}</li>
												</ul>
											</div>
										</div>
										<div class="operation">
											<a class="items-delete-btn" @click="delCarPanelHandle(CarGoogs.sku_id)"></a>
										</div>
										<div class="subtotal">¥ {{CarGoogs.count*CarGoogs.price}}</div>
										<div class="item-cols-num">
											<div class="select js-select-quantity">
												<span class="down" :class = "{'down-disabled': CarGoogs.count === 1 }" @click="subCarPanelDataHandle(CarGoogs.sku_id)">-</span>
												<span class="num">{{CarGoogs.count}}
												...
												<span class="up" :class = "{'up-disabled': CarGoogs.count === CarGoogs.limit_num }" @click="plusCarPanelDataHandle(CarGoogs.sku_id)">+</span>
												
											</div>
										</div>
										<div class="price">¥ {{CarGoogs.price}}.00</div>
									</div>
								</div>
					...
					<div class="shipping">
						<div class="shipping-box">
							<div class="shipping-total shipping-num">
								<h4 class="">
									已选择 <i>{{checkedCountHandle}}</i> 件商品
								</h4>
								<h5>
									共计 <i >{{count}}</i> 件商品
								</h5>
							...
							<div class="shipping-total shipping-price">
								<h4 class="">
									应付总额：<span>￥</span><i >{{checkedPriceHandle}}</i> 
								</h4>
            ...
            <router-link tag="span" class="jianguo-blue-main-btn big-main-btn js-checkout " :class = "{'disabled-btn': checkedCountHandle===0}" :to = "{name:'CheckOut'}"><a>现在结算</a></router-link>
